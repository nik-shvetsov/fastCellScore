...
from sympy import Point, Ray, Line, Segment, Polygon, deg, sqrt, cos, sin, N, solve
...
def extract_slide_info(slide_path):
    ...
    if "aperio.MPP" not in ref_slide.get_fields():
        raise ValueError("No MPP field ('aperio.MPP') in slide %s" % slide_path)
...
def find_point_on_line(p1, p2, threshold):
    ...
    new_point = point1 + vector_norm * threshold
...
def generate_tissue_mask(
    ...
    # downscaled_mask = cv2.resize(uint_mask, (slide_lowres.width, slide_lowres.height), interpolation=cv2.INTER_AREA)
    # cv2.imwrite(f"{slide_info['name']}_debug_mask.png", downscaled_mask * 255)
        ...
        if roi_mask is not None:
            ...
            cords_vips_patch_candidates = [
                ...
                if (
                    ...
                    # and is_intensity_low(cords_vips_patch_candidates[i][1])
                    # and is_bw_not_empty(cords_vips_patch_candidates[i][1])
                )
            ]
        else:
