...
from sympy import (
    Point, Point2D, Ray, Line, Segment, Polygon, deg, sqrt, cos, sin, N, solve
)
...
def plot_patches_on_slide(slide_info, points_pairs_list):
    ...
    for points_pair in points_pairs_list:
        scaled_points_pair = scale_coords(points_pair, slide_info['raw_size'], (slide_vips.width, slide_vips.height))
...
def get_annotated_image(slide, annotations, level, fill=False, color=(0, 255, 0), lt=5, verbose=False):
    '''
    ...
    res_img = draw_all_polys(np.asarray(pil_img), scaled_polys, fill=fill, color=color, lt=lt)
...
def get_annotated_image_vips(slide_path, slide_info, l0_annotations, level=None, fill=False, color=(0, 255, 0), lt=5, verbose=False):
    '''
    slide: vips object
    level: downsampling level from 1 to 4 usually, maps to downsampling factor in `slide.level_downsamples`
    annotations: points, obtained from annotation `.itn` file
...
def visualize_contours(slide_path, slide_info, l0_annotations, save_file_path, level=None, fill=False, color=(0, 255, 0), lt=5):
    '''
    slide: vips object
    level: downsampling level from 1 to 4 usually, maps to downsampling factor in `slide.level_downsamples`
    annotations: points, obtained from annotation `.itn` file
...
def extract_patches_btw_points(img, p1, p2, patch_size=748):
    ...
    rotate_matrix = cv2.getRotationMatrix2D(center=center, angle=math.degrees(angle), scale=1)
    ...
    for idx, line_stop in enumerate(line_stops):
        ...
        if n_full_patches > 0:
            ...
            if idx == len(line_stops)-1 and residual != 0:
                crop_img = rotated_image[y:y+patch_size, x+patch_size:(x+patch_size)+residual]
                rimgs.append(crop_img)
        else:
...
def extract_slide(slide_path, slide_info, add_alpha=False, level=None):
    ...
    slide = pyvips.Image.new_from_file(slide_path, level=level).extract_band(0, n=n_bands)
...
def scale_polygons_coords(reference_coords, reference_size, target_size):
    ...
    for k in reference_coords.keys():
        ...
        for item in reference_coords[k]:
            updated_item = np.floor((np.asarray(item).ravel() * target_size) / reference_size).astype("int64")
...
def _reduce_polygons_points(polygons, threshold, close=True):
    ...
    for k, polygon in polygons.items():
        ...
        if len(set(map(tuple, result[k]))) == 1:
            np0, np1 = map(lambda x: (x + threshold / 2 * np.array([-1, 1])).tolist(), [result[k][0]] * 2)
            result[k] = [np0, np1]
        ...
        if fix_last_missing: 
            ...
            if distance != 0.0:
                while distance > threshold:
                    updated_point = find_point_on_line(last_point, first_point, threshold)
                    reduced_polygon.append(updated_point)
...
def get_points_pair_from_slide_reduced_polys(slide, reduced_polys, threshold=768):
    upd_reduced_polys = {}
    ...
    for k in upd_reduced_polys.keys():
        ...
        distance = np.linalg.norm(np.array(last_point, dtype=np.float32) - np.array(first_point, dtype=np.float32))
        if distance > (threshold / 3):
            upd_last_point, upd_first_point = calculate_closing_patch_points(first_point, last_point, threshold)
            upd_reduced_polys[k] = (
                upd_reduced_polys[k][0], 
                (
                    Point(upd_last_point),
                    Point(upd_first_point)
                )
            )
    ...
    # Add index
    slide_with_point_pair_list = [(idx,) + el for idx, el in enumerate(slide_with_point_pair_list)]
...
def rotate_point(orig_shape, rot_shape, point, angle_rad):
    ...
    '''
    org_center = np.array(orig_shape) / 2.
    rot_center = np.array(rot_shape) / 2.
...
def extract_patch(vips_img, pts, patch_height):
    ...
    if (int(bbox_points[0].x) + int(width) > vips_img.width or
        ...
        dlt = calculate_extend_polygon((bbox_points[0], bbox_points[1], bbox_points[2], bbox_points[3]), (vips_img.width, vips_img.height), eps=patch_height//2)
...
def is_row_high_intensity(vips_crop_from, pts_pair, patch_height, ms={"mean": 210, "std": 30}): 
...
def is_hematoxilin_enough_normed_H(vips_patch_crop, normalizer, tf, threshold=0.017):
    ...
    masked_img = np.where(binary_mask[..., None], vips_patch_crop.numpy(), 255).astype(np.uint8)
    # apply normalization ?
...
def is_intensity_low(vips_patch_crop, ms={"mean": 210, "std": 30}):
    ...
    """
    return bool(vips_patch_crop.avg() < ms["mean"] and vips_patch_crop.deviate() > ms["std"])
...
def calculate_norm_mm2(slide_mpp, patch_size, abs_counts):
    ...
    cell_density_in_cells_per_mm_sq = cell_density_in_cells_per_micron_sq * conversion_factor
    return cell_density_in_cells_per_mm_sq

def calculate_cells_score(slide_info, cell_types, s4_dataframe, patch_size=256, drop_zero=False):
    result = {}
    dfo = s4_dataframe.clone()
    for cell_type in cell_types:
        norm_col = f'{cell_type}_norm_mm2'
        # Get normalized values using mpp for each sub-patch
        dfo = dfo.with_columns(pl.col(cell_type).map_elements(lambda value: calculate_norm_mm2(slide_info['mpp'], patch_size, int(value)), return_dtype=pl.Float64).alias(norm_col))

        # Summarize norm_mm2 column by "Pair_ID"
        dfo_sum = dfo.group_by(['Pair_ID']).agg([pl.col(norm_col).sum().alias(norm_col)])
        ...
        dfo_sum = dfo_sum.with_columns((pl.col(norm_col) / pl.col('count')).alias(norm_col))
...
def generate_tissue_mask(
    ...
    try:
        slide_lowres = extract_slide(slide_path, slide_info, level=extraction_level).numpy()
        slide_lowres = extract_slide(slide_path, slide_info, level=extraction_level)
    except:
        slide_lowres = extract_slide(slide_path, slide_info, level=extraction_level-1).numpy()
    ...
    l0_polys = scale_polygons_coords(
        reference_polygons, reference_size, target_size
    )
    mask = np.zeros((slide_info["raw_size"][1], slide_info["raw_size"][0]), dtype=np.uint8)    
...
def grid_H_patch_coord_generator(
    ...
    """
    """
    print (f"Using 'grid_H_patch_coord_generator' with {sort_h_min_threshold} threshold.")
    ...
    vips_patch_candidates = [vips_image.crop(left, top, width, height) for left, top, width, height in patch_candidates]
    cords_vips_patch_candidates = list(zip(patch_candidates, vips_patch_candidates))

    ### Mask definition #####################
    if not ignore_tissue_mask:
        mask, tissue_countours = generate_tissue_mask(slide_path, save_tissue_contour_image=save_plot)  
    #########################################
    
    if mask is not None:
        cords_vips_patch_candidates = [cords_vips_patch_candidates[i] for i in range(len(cords_vips_patch_candidates)) if 
        (   
            is_within_mask(cords_vips_patch_candidates[i][0], mask, mask_threshold)
        )
        ]

    if sort_h_min_threshold is not None:
        patch_candidates_with_hema_all = [(x[0], get_hematoxilin_component(x[1])) for x in cords_vips_patch_candidates]
        patch_candidates_with_hema_all = sorted(patch_candidates_with_hema_all, key=lambda x: x[1], reverse=True)

        patch_candidates_with_hema_cut = [x for x in patch_candidates_with_hema_all if x[1] > sort_h_min_threshold]
    ...
    n_to_yield = max(
        int(
            min(tumor_to_tissue_area_ratio, 1.0) * len(patch_candidates_cut)
        ), min_num_patches
    )
    ...
    print (f"Total patches: {n_total_patches}, cut: {len(patch_candidates_cut)} Patches to yield: {tumor_to_tissue_area_ratio} * {len(candidates_to_yield)} : {n_to_yield}")
...
def grid_patch_coord_generator(
    ...
    if logger is not None:
        if isinstance(to_yield, float):
            logger.info(f"Using 'grid_patch_coord_generator' for to_yield = {to_yield} ({to_yield * 100:0.2f}%).")
        elif isinstance(to_yield, int):
            logger.info(f"Using 'grid_patch_coord_generator' for to_yield = {to_yield} (int). Consider using 'grid_patch_coord_generator' for float ratio.")
    ...
    if perform_checks:
        if roi_mask is not None:
            cords_vips_patch_candidates = [cords_vips_patch_candidates[i] for i in range(len(cords_vips_patch_candidates)) if 
            (   
                is_within_mask(cords_vips_patch_candidates[i][0], roi_mask, mask_threshold)
            )
            ]
            ...
            ### If we have a generated mask, we drasticly reduce the number of candidates before checks, and thus speed up the process of checking candidates
                tissue_mask, _ = generate_tissue_mask(slide_path, save_tissue_contour_image=save_plot)

                cords_vips_patch_candidates = [cords_vips_patch_candidates[i] for i in range(len(cords_vips_patch_candidates)) if 
                (   
                    is_within_mask(cords_vips_patch_candidates[i][0], tissue_mask, mask_threshold)
                )
                ]
    ...
    if sort_h is not None:
        patch_candidates_with_hema = [(x[0], get_hematoxilin_component(x[1])) for x in cords_vips_patch_candidates]
        patch_candidates = sorted(patch_candidates_with_hema, key=lambda x: x[1], reverse=True)
    ...
    if logger is not None:
        logger.info(f"Patches to yield: {n_patches_to_yield}, Total eligible patches: {len(patch_candidates)}, Total patches: {n_total_patches}")
...
def fast_grid_patch_coord_generator(
    ...
    if logger is not None:
        if isinstance(to_yield, float):
            logger.info(f"Using 'fast_grid_patch_coord_generator' for to_yield = {to_yield} ({to_yield * 100:0.2f}%). Consider using 'grid_patch_coord_generator' for float ratio.")
        elif isinstance(to_yield, int):
            logger.info(f"Using 'fast_grid_patch_coord_generator' for to_yield = {to_yield} (int).")
    ...
    vips_patch_candidates = [vips_image.crop(left, top, width, height) for left, top, width, height in all_coord_pivots]
    ...
    if logger is not None:
        info_n_patches = f"total WSI patches" if mask is None else "eligible patches, using mask"
        logger.info(f"Patches to yield: {n_patches_to_yield} out of {n_total_patches} {info_n_patches}")
    
    approved_candidates = []
    all_patches_idx = 0

    while len(approved_candidates) < n_patches_to_yield and all_patches_idx < len(cords_vips_patch_candidates):
        candidate_subset = cords_vips_patch_candidates[all_patches_idx:all_patches_idx + (n_patches_to_yield - len(approved_candidates))]
        all_patches_idx += n_patches_to_yield
        
        if perform_checks:
            approved_subset_candidates = [candidate_subset[i] for i in range(len(candidate_subset)) if 
            (   
                is_hematoxilin_enough(candidate_subset[i][1])
            )
            ]
    ...
    ### Sort by hematoxylin in descending order
    if sort_h:
        patch_candidates_with_hema = [(x[0], get_hematoxilin_component(x[1])) for x in approved_candidates[:n_patches_to_yield]]
        patch_candidates = sorted(patch_candidates_with_hema, key=lambda x: x[1], reverse=True)
        patch_candidates = [x[0] for x in patch_candidates]      
    else:
...
def normH_patch_coord_generator(
    ...
    if logger is not None:
        if isinstance(to_yield, float):
            logger.info(f"Using 'normH_patch_coord_generator' for to_yield = {to_yield} ({to_yield * 100:0.2f}%). Consider using 'grid_patch_coord_generator' for float ratio.")
        elif isinstance(to_yield, int):
            logger.info(f"Using 'normH_patch_coord_generator' for to_yield = {to_yield} (int).")
    ...
    if perform_checks:
        ...
        normalizer = NormalizerBuilder.build('reinhard', concentration_method='ls').to('cpu')
        ref_image = cv2.resize(cv2.cvtColor(cv2.imread(
            str(Path("src", "patch_class", "patch_class", "img_ref", "1_ref_img.png"))
        ), cv2.COLOR_BGR2RGB), (patch_size, patch_size))
    ...
    while len(approved_candidates) < n_patches_to_yield and all_patches_idx < len(cords_vips_patch_candidates):
        ...
        if perform_checks:
            approved_subset_candidates = [candidate_subset[i] for i in range(len(candidate_subset)) if 
            (   
                is_hematoxilin_enough_normed_H(candidate_subset[i][1], normalizer, tfs)
            )
            ]
            approved_candidates.extend(approved_subset_candidates)
        else:
...
def visualize_patches(
    ...
    if include_classes is not None and 'class' in pd_pts.columns:
        legend_elements = [Patch(facecolor=[i/255 for i in cmap[str(cls)][1]], edgecolor='w', label=cmap[str(cls)][0]) for cls in include_classes]
